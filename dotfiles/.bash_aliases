...
if [[ `alias bash_aliases_pragma_once 2>&1 | grep -c "bash: alias"` != 0 ]]; then
  ...
  alias cmakedebug='Fortran_FLAGS=-Werror CUDA_FLAGS=-Werror\ all-warnings C_FLAGS=-Werror CXX_FLAGS=-Werror cmake -GNinja -DCMAKE_BUILD_TYPE=Debug -Wdeprecated -Wdev'
